...
class StatusBarDataSaverUtil(private var mContext: Context) {
    var isExpand = false
        private set
    ...
    companion object {
        private var INSTANCE: StatusBarDataSaverUtil? = null
        fun getInstance(context: Context): StatusBarDataSaverUtil? {
            if (INSTANCE == null) {
                return StatusBarDataSaverUtil(context).also { INSTANCE = it }
            }
        ...
        fun setupStatusBar(
            ...
            if (getInstance(context)!!.isExpand) {
                ...
                val isEnableDataSaverSystem = Settings.Global.getInt(
                    context.contentResolver,
                    Settings.Global.AIRPLANE_MODE_ON, 0
                ) != 0
                val isEnableDataSaver: Boolean =
                    getInstance(context)!!.setting.isEnableDataSaver // cua app minh muon set

                if (isEnableDataSaver != isEnableDataSaverSystem) {
                    DataSaverFilterUtil.setEnable(
                        context,
                        nodeInfo,
                        isEnableDataSaver,
                        listenerEndWindow
                    )
                    getInstance(context)!!.setting.isEnableDataSaver = (isEnableDataSaver)
                }
